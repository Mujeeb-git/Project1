...
public class ExcelOperations {
	...
	public String readExcel(String sheetName, int rowNum, int colNum) {
		try {
			...
			File src = new File(System.getProperty("user.dir") + "\\DataSource\\Workbook1.xlsx");
	...
	public void writeExcel(String sheetName, int rowNum, int colNum) {
		...
		try {
			...
			sh1.getRow(rowNum).createCell(colNum).setCellValue(sh1.getRow(rowNum).getCell(colNum).getStringCellValue());

			// here we need to specify where you want to save file

			FileOutputStream fout = new FileOutputStream(
					new File(System.getProperty("user.dir") + "\\DataSource\\Workbook2.xlsx"));
